...
\begin{figure}[h]
  ...
  \label{fig:rting-tbl}
\end{figure}
What is the important information that is missing to route? % The metric
\subsection{Routing}
\subsubsection{New route}
A router just got an update with theses networks: 57.6.96.0/21, 57.6.104.0/21, 57.6.112.0/21, 57.6.120.0/21. If the routes uses the same NIC, can they be summarized? %57.6.96.0/19
\subsubsection{New network}
A router has a route for the range 29.18.0.0 to 29.18.128.255 into the route 29.18.0.0/17. A new 1024-addresses network, from 29.18.60.0 to 29.18.63.255, got advertised on a new interface of the router. Should the CIDR be spread out? %Nope, the new route is 29.18.0.0/22, if a packet comes for this network, the submask is bigger than the 29.18.0.0/17, so the better match will be for the new route.
...
Lot of firm use two routers in case of one crashes. Is the NAT-ing possible? % Assuming that the two routers have their own IP address: yes, and the packets must not be dispatched between the two routers.
