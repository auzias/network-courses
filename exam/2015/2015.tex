...
\section{OSI model (2 pts)}
	Give the name and a short overview of the four first layers:%cf slides
\pagebreak

\section{Routing decision (2 pts)}
	\subsection{Definitions}
		What are the definitions of: \emph{router}, \emph{routing algorithm} and \emph{forwarding}? (You can make up these three definitions in one sentence).%cf slides
\vspace{3cm}
	...
	\subsection{Next NIC?}
		...
		\begin{itemize}
			\item What are the (optional) missing details ?%optional: gateway (the interface is enough though), not optional: metric. Flags was also accepted as optional.
			\item Where will be forwarded packet with destinations:
			\begin{itemize}
				\item 198.41.191.47?	%R0
				\item 192.168.4.3?		%R0
				\item 192.168.1.1?		%R0 !
				\item 80.10.201.0?		%R3
				\item 80.10.210.0?		%R0
				...
				\item 212.27.61.1?		%R2
			\end{itemize}
			\item Is there routes than can be summarized ? If yes, which one(s) into which "super-network"? % No ! There isn't any. If you summarized 192.168.0.0/24 and 192.168.5.0/25 then 192.168.1.0/23 (both .1.0/24 and .2.0/24) will be then be forwarded to this summarized route and not R0 anymore!
		\end{itemize}

\pagebreak
\section{What layer is it ? (2 pts)}
	Complete the table \ref{tab:protocol} with (include the default port, if any, in brackets -see HTTP example-): \sout{HTTP}, HTTPS, TCP, UDP, MAC, IP, telnet, ssh, ftp, IEEE 802.11, IEEE 802.15.2, DNS, SIP, TLS, ICMP, IS-IS, RIP. %Cf the RFC (or wikipedia). IS-IS is a routing protocol, so is RIP, then it's layer 3.
	\subsection{Next NIC?}
	...
	\section{Who is it? (2 pts)}
		What can you conclude according to the two packets shown in \ref{fig:dns}? %Client is.... Destination is... It's a DNS query for the website 9gag.com that responds to two IP addresses.
		\begin{figure}[h]
			\begin{center}
...
\section{Bonus question (0.5 pts)}
	\subsection{DEF CON 22 Hacking Conference}
		What did you learn by watching to the video named \emph{Blinding The Surveillance State By Christopher Soghoian}? %Did you really watch the video or just remember what I say during the lesson?
\pagebreak
