...
\subsubsection{TCP}
Use the mode TCP of \verb"netcat" and try it. Can \verb"netstat" could, somehow, be handful for anything while waiting for connection? Can \verb"telnet" be used to chat?
\subsubsection{UDP}
Use the mode UDP of \verb"netcat" and try it. Explain a situation within the server could not receive every packet.
Try to produce this situation by implementing it using Java langage. (\verb"InetAddress" and \verb"DatagramPacket" should be useful...)

More example of \color{blue}\href{http://www.brianhaddock.com/2013/8-great-netcat-nc-unix-commands}{netcat}\color{black}
\end{document}
